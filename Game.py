...
path = str(Path.cwd())
Background = cv2.imread(path +"\\images\\4729668.png",cv2.IMREAD_UNCHANGED)
Background = cv2.resize(Background, (1280,720))
PaddleLeft = cv2.imread(path + "\\images\\unnamed.png",cv2.IMREAD_UNCHANGED)
PaddleLeft =cv2.resize(PaddleLeft,(100,100))
PaddleRight = cv2.imread(path + "\\images\\unnamed.png",cv2.IMREAD_UNCHANGED)
PaddleRight =cv2.resize(PaddleRight,(100,100))
Puck =cv2.imread(path + "\\images\\share-it-green-glowing-circle-11563079981sivw5upspu.png",cv2.IMREAD_UNCHANGED)
...
while True:
    ...
    if hands:
        for hand in hands:
            ...
            if hand['type'] == "Left":
                img = htm.overlayPNG(img, PaddleLeft, (x1, y1))
            ...
            if hand['type'] == "Right":
                img = htm.overlayPNG(img, PaddleRight, (x2, y2))
    ...
    if gameOver:
        imgGameOver = cv2.imread(path + "\\images\\gameOver.png")
    ...
    # If game not over move the ball
        ...
        # Draw the ball
        img = htm.overlayPNG(img, Puck, puckPos)
